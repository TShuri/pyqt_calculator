...
class Logic():
    ...
    def run(self, event=None, files=None):
        ...
        filepath = files
        self.output("Расчет...\n")
        for p, i in enumerate(filepath):
            self.output(f"{'#' * 30} Обязательство {p+1} {'#' * 30}")
            kol_obyz = kol_obyz + 1
            try:
                wb = load_workbook(i, data_only=True)
            except:
                df = pd.read_excel(i)
                df.to_excel(i, index=False, header=False)
                wb = load_workbook(i, data_only=True)
        ...
        if pod > 0:
            self.output(f'{pod:.0f} - просроченный основной долг')
        if rcy > 0:
            self.output(f"{'-' * 40}")
